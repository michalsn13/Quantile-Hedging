    ...
    def payoff_special(self, X_t, X0_nt):   
        ...
        if self.target_function == 1:
            ...
            def f(i):
                ...
                print((G_delta(10000, mu_nt, sigma_nt, rho, B_T[i], self.K, self.T, X0_nt, dP_dQ[i], self.call), B_T[i]))
                return root_scalar(wrapping_function, args = (mu_nt, sigma_nt, rho, B_T[i], self.K, self.T, X0_nt, dP_dQ[i], self.call, self.m), bracket= (left_bound, right_bound), method = "bisect", rtol= 0.0000001).root
            print("ELO")
